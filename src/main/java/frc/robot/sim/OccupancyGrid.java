...
/**
 * Log-odds occupancy grid for real-time mapping from ultrasonic sensor readings.
 * Uses an inverse sensor model with Bresenham ray traversal to mark cells as
 * free (along the beam) or occupied (at the endpoint).
 *
...
public class OccupancyGrid {
  ...
  /** Grid origin X in world frame (inches) */
  private final double originX;

  /** Grid origin Y in world frame (inches) */
  ...
  /** Log-odds values for each cell. 0.0 = unknown, positive = occupied, negative = free */
   ...
   *
   * @param worldWidth  World width in inches
   * @param worldHeight World height in inches
   * @param resolution  Cell size in inches (e.g. 2.0 = 2 inch cells)
   */
  public OccupancyGrid(double worldWidth, double worldHeight, double resolution) {
    this.resolution = resolution;
    this.originX = 0.0;
    this.originY = 0.0;
    this.gridWidth = (int) Math.ceil(worldWidth / resolution);
    this.gridHeight = (int) Math.ceil(worldHeight / resolution);
    this.logOdds = new double[gridWidth][gridHeight];
  }
   ...
   *
   * @param range      Measured range in inches
   * @param robotX     Robot X position (inches)
   * @param robotY     Robot Y position (inches)
   * @param robotTheta Robot heading (radians)
   */
  public void update(double range, double robotX, double robotY, double robotTheta) {
    ...
    boolean isMaxRange = (range >= MAX_RANGE - 1.0);
   ...
   * Cells near the endpoint are marked occupied (unless max range = no hit).
   */
  private void updateRay(double sensorX, double sensorY, double rayAngle,
                          double range, boolean isMaxRange) {
    ...
    while (true) {
      if (inBounds(cx, cy)) {
        if (step < freeStopStep || isMaxRange) {
          // Mark as free
          logOdds[cx][cy] = clamp(logOdds[cx][cy] + L_FREE);
        }
      }
    ...
    // Mark endpoint region as occupied (only if not max range)
    if (!isMaxRange) {
  ...
  /**
   * Resets the entire grid to unknown (all log-odds = 0).
   */
  public void reset() {
    for (int gx = 0; gx < gridWidth; gx++) {
      for (int gy = 0; gy < gridHeight; gy++) {
  ...
  /** Gets the grid origin X. */
  public double getOriginX() {
    return originX;
  }

  /** Gets the grid origin Y. */
